#!/bin/bash
#  ----------------------------------------------------------------------------------
#  curl-vault-write-key.sh version 1.0 created February 16, 2018
#
...
if [[ $# != 3 ]]; then
        echo "+++ SYNTAX ERROR"
        usage "<vault-server-id (s1|s2)> <key-name> <key-value>"
        exit
fi
...
echo ">> Create a secret: ${_KEY}=${_VALUE} @${_SERVER}:${_PORT}"
