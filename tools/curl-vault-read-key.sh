#!/bin/bash
#  ----------------------------------------------------------------------------------
#  curl-vault-read-key.sh version 1.0 created February 14, 2018
#
...
echo ">> Read a secret: ${_KEY} @${_SERVER}:${_PORT}"

curl -sw 'HTTP STATUS: %{http_code}\n' ${_TLS} ${_REDIRECT} \
  --header "X-Vault-Token: ${_ROOT_TOKEN}" \
  --request GET \
  ${_HTTP_SCHEME}://${_SERVER}:${_PORT}/v1/secret/${_KEY} >${_TMP}

if [[ $(grep 404 ${_TMP}) ]]; then
   echo "+++ HTTP STATUS 404: Key not found"
fi

grep -v "HTTP STATUS:" ${_TMP} | jq
rm -f ${_TMP} 
